...
public class LatestSIPList extends AppCompatActivity
{
    ...
    private ArrayList<Last30DaysDataResponse.SipStpDetailsItem> list30DaysDataList = new ArrayList<Last30DaysDataResponse.SipStpDetailsItem>();
    ...
    private void initView()
    {
        try
        {
            ...
            txtTitle.setText("Latest SIP - Last 30 days");
    ...
    private void getLast30DaysData()
    {
        list30DaysDataList =  new ArrayList<>();
                    ...
                    if(response.body().getSuccess() == 1)
                    {
                        try
                        {
                            if (response.body().getSipStpDetails().size() > 0)
                            {
                                ...
                                list30DaysDataList.addAll(response.body().getSipStpDetails());
                                if(list30DaysDataList.size() > 1)
                                {
                                    llNoData.setVisibility(View.GONE);
                                    rvSPIData.setAdapter(new SIPDataAdapter(list30DaysDataList));
                                }
                                else
                                {
    ...
    {
        ArrayList<Last30DaysDataResponse.SipStpDetailsItem> listItem;

        SIPDataAdapter(ArrayList<Last30DaysDataResponse.SipStpDetailsItem> listData)
        {
            this.listItem = listData;
        }

        @Override public SIPDataAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i)
        {
            View v = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.portfolio_row_view_sip_data_temp, viewGroup, false);
            return new SIPDataAdapter.ViewHolder(v);
        }

        @Override public void onBindViewHolder(final SIPDataAdapter.ViewHolder holder, final int position)
        {
            try
            {
                final Last30DaysDataResponse.SipStpDetailsItem getSet = listItem.get(position);
