...
class FinPlanVarianceAnalysisMicroStrategicListActivity : FinPlanBaseActivity()
{
    ...
    {
        tvTitle.text = "Variance Analysis(Micro-Strategic)"
        ivBack.backNav()

        tvNoData.text = "Variance Analysis(Micro-Strategic) not found!"
        rvAspiration.layoutManager = LinearLayoutManager(activity)

        ivGraph.setOnClickListener {
            startActivity<FinPlanGraphActivity>(AppConstant.camefrom.CAME_FROM to AppConstant.camefrom.GRAPH_VARIANCE_MICRO_STRATEGIC, "data" to Gson().toJson(listAspiration))
        }
    }
    ...
    private fun getNetWorthList()
    {
        ...
        {
            loader.show()
            apiService.getVarianceAnalysisMicroStrategic(sessionManager.userId).enqueue(object : Callback<VarianceAnalysisMicroListResponse>
                {
