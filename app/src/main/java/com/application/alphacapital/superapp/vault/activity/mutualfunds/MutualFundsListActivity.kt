...
class MutualFundsListActivity : VaultBaseActivity()
{

    private lateinit var binding: VaultActivityGovRelatedListBinding
    var listNotification: MutableList<MutualFundsListResponse.MutualFund> = mutableListOf()
    ...
    {
        binding.toolbar.txtTitle.text = "Mutual Funds"
    ...
    private fun getListData()
    {
        try
        {
            if (isOnline())
            {
                ...
                val call = apiService.getmutual_fundsCall(sessionManager.userId, "", "", "")

                call.enqueue(object : Callback<MutualFundsListResponse>
                {
                    override fun onResponse(call: Call<MutualFundsListResponse>, response: Response<MutualFundsListResponse>)
                    {
                        if (response.isSuccessful)
                        {
                            if (response.body()!!.success == 1)
                            {
                                ...
                                listNotification.addAll(response.body()!!.mutual_funds)
                    ...
                    override fun onFailure(call: Call<MutualFundsListResponse>, t: Throwable)
                    {
                        binding.loading.llLoading.visibility = View.GONE
                        apiFailedToast()
                       // visible(binding.noData.llNoData)
    ...
    inner class NotificationListAdapter : RecyclerView.Adapter<NotificationListAdapter.ViewHolder>()
    {
        ...
        {
            val getSet: MutualFundsListResponse.MutualFund = listNotification[position]
            ...
            holder.itemView.ivDelete.setOnClickListener {
                if (isOnline())
                {
                    deleteDialog(getSet.mutual_funds_id, position)
                }
                else
                {
                    noInternetToast()
            ...
            holder.itemView.ivEdit.setOnClickListener {
                if (isOnline())
                {
                    val intent = Intent(activity, AddMutualFundsActivity::class.java)
                    ...
                    intent.putExtra("data", Gson().toJson(getSet, MutualFundsListResponse.MutualFund::class.java))
    ...
    {
        UniversalAlertDialog(activity, "Mutual Funds", "Are you sure you want to delete this details?", object : DialogActionInterface
        {
    ...
    private fun deletelistItems(govRelatedId: String, pos: Int)
    {
        try
        {
            if (isOnline())
            {
                hideKeyBoard()
                binding.loading.llLoading.visibility = View.VISIBLE
                val call = apiService.deletemutual_fundsCall(govRelatedId)
