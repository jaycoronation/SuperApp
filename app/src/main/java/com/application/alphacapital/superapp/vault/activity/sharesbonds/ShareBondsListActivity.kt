...
import com.alphaestatevault.model.ShareBondsListResponse
...
class ShareBondsListActivity : VaultBaseActivity()
{

    private lateinit var binding: VaultActivityGovRelatedListBinding
    var listNotification: MutableList<ShareBondsListResponse.SharesBond> = mutableListOf()
    ...
    {
        binding.toolbar.txtTitle.text = "Share Bonds"
    ...
    private fun getListData()
    {
        try
        {
            if (isOnline())
            {
                ...
                val call = apiService.getshares_bondsCall(sessionManager.userId, "", "", "")

                call.enqueue(object : Callback<ShareBondsListResponse>
                {
                    override fun onResponse(call: Call<ShareBondsListResponse>, response: Response<ShareBondsListResponse>)
                    {
                        if (response.isSuccessful)
                        {
                            if (response.body()!!.success == 1)
                            {
                                ...
                                listNotification.addAll(response.body()!!.shares_bonds)

                                if (listNotification.size > 0)
                                {
                                    binding.rvList.adapter = notificationListAdapter
                                    //gone(binding.noData.Safe Deposit Boxes)
                    ...
                    override fun onFailure(call: Call<ShareBondsListResponse>, t: Throwable)
                    {
                        binding.loading.llLoading.visibility = View.GONE
                        apiFailedToast()
                       // visible(binding.noData.llNoData)
    ...
    inner class NotificationListAdapter : RecyclerView.Adapter<NotificationListAdapter.ViewHolder>()
    {
        ...
        {
            val getSet: ShareBondsListResponse.SharesBond = listNotification[position]
            ...
            holder.itemView.ivDelete.setOnClickListener {
                if (isOnline())
                {
                    deleteDialog(getSet.shares_bonds_id, position)
                }
                else
                {
                    noInternetToast()
            ...
            holder.itemView.ivEdit.setOnClickListener {
                if (isOnline())
                {
                    val intent = Intent(activity, AddShareBondsActivity::class.java)
                    ...
                    intent.putExtra("data", Gson().toJson(getSet, ShareBondsListResponse.SharesBond::class.java))
    ...
    {
        UniversalAlertDialog(activity, "Share Bonds", "Are you sure you want to delete this details?", object : DialogActionInterface
        {
    ...
    private fun deletelistItems(govRelatedId: String, pos: Int)
    {
        try
        {
            if (isOnline())
            {
                hideKeyBoard()
                binding.loading.llLoading.visibility = View.VISIBLE
                val call = apiService.deleteshares_bondsCall(govRelatedId)
